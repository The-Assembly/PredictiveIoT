...
module("handlers.http", thingworx.handler.extend)
...
function read(me, pt, headers, query)
  ...
  local code, resp = makeHttpRequest(method, pt.key, headers, false)
...
function write(me, pt, headers, query, data)
  ...
  return makeHttpRequest(method, pt.key, headers, json.encode(data), false)
end
